...
// GetBatchChannel バッチチャネルを返す（GetBatchReadyの別名）
func (ab *AudioBatcher) GetBatchChannel() <-chan *model.AudioBatch {
	return ab.batchReady
}

// StartBatching バッチ処理を開始
func (ab *AudioBatcher) StartBatching(ctx context.Context) {
	// タイムアウト監視のためのタイマーを開始
	go ab.startFlushTimer(ctx)
	log.Println("音声バッチ処理を開始しました")
}

// startFlushTimer タイムアウト監視タイマーを開始
func (ab *AudioBatcher) startFlushTimer(ctx context.Context) {
	ticker := time.NewTicker(ab.flushTimeout)
	defer ticker.Stop()
...
// checkTimeouts タイムアウトしたバッチをチェックしてフラッシュ
func (ab *AudioBatcher) checkTimeouts() {
	ab.mu.Lock()
	defer ab.mu.Unlock()
